...
将棋ウォーズ.棋譜ページ解析 = function(棋譜ID){
    ...
    var 先手名 = 棋譜ID.split(/-/)[0] + ' ' + (ソース.match(/dan0: "(.+?)"/)[1] || '');
    var 後手名 = 棋譜ID.split(/-/)[1] + ' ' + (ソース.match(/dan1: "(.+?)"/)[1] || '');
    var csa    = ソース.match(/receiveMove\("(.+?)"/)[1];

    将棋ウォーズ.KIF保存(棋譜ID, csa, 先手名, 後手名);
};



将棋ウォーズ.KIF保存 = function(棋譜ID, csa, 先手名, 後手名){
    var kif = "";
    kif += "開始日時：" + 将棋ウォーズ.棋譜ID→時間(棋譜ID) + "\r\n";
    kif += "先手：" + 先手名 + "\r\n";
    kif += "後手：" + 後手名 + "\r\n";
    kif += "棋戦：将棋ウォーズ " + 将棋ウォーズ.現在のモード + "\r\n";
    kif += "場所：https://kif-pona.heroz.jp/games/" + 棋譜ID + "\r\n";
    kif +=  将棋ウォーズ.KIF変換(csa);
